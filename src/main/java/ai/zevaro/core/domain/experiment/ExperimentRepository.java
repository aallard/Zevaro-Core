...
public interface ExperimentRepository extends JpaRepository<Experiment, UUID> {
    ...
    Page<Experiment> findByTenantIdAndProgramId(UUID tenantId, UUID projectId, Pageable pageable);

    List<Experiment> findByTenantIdAndStatus(UUID tenantId, ExperimentStatus status);

    List<Experiment> findByTenantIdAndProgramId(UUID tenantId, UUID projectId);

    List<Experiment> findByTenantIdAndProgramIdAndStatus(UUID tenantId, UUID projectId, ExperimentStatus status);

    List<Experiment> findByHypothesisId(UUID hypothesisId);

    @Query("SELECT COUNT(e) FROM Experiment e WHERE e.tenantId = :tenantId AND e.program.id = :projectId")
    long countByTenantIdAndProjectId(@Param("tenantId") UUID tenantId, @Param("projectId") UUID projectId);

    @Query("SELECT COUNT(e) FROM Experiment e WHERE e.tenantId = :tenantId AND e.program.id = :projectId AND e.status = :status")
    ...
    @Query("SELECT COUNT(e) FROM Experiment e WHERE e.tenantId = :tenantId AND e.program.id = :projectId AND e.status IN ('RUNNING', 'IN_PROGRESS', 'ACTIVE')")
